...
<body>
    ...
    <h1>登録フォーム</h1>
    <h2>下記、必要事項をご確認いただきご記入の上で送信ください。</h2>
    ...
    <!-- ------------------------------------------------------ -->
        ...
        <!-- textaea：詳細 -->
        <div class="col">詳細
            <textarea name="detail" cols="30" rows="10" class="form_parts" id='input_textarea' required></textarea>
        </div>
        ...
        <!-- [初期に戻る/クリア]ボタン -->
        <input type="button" value="クリア" clss="btn_submit" onclick="location.href='./post.php'" >
...
<script type="text/javascript">
    ...
    // 先に入力のデータを復活させるため、LocalStorageを活用
    let values = localStorage.getItem(0);
    values = values.split(',');

    // 先に入力の値に応じて、フォームの各パーツの値を入力
    $("#input_name").val(values[0]);
    $("#input_email").val(values[1]);
    $("#select_trigger option[value='"+values[2]+"']").prop('selected', true);
    $("#check"+values[3].split('_')[0]+"").prop("checked",true);
    $("#input_textarea").val(values[4]);
